...
$.fn.getGridHeight = function (hasPage) {
    ...
    if (hasPage) {
        pageHeight = 55;//55代表分页高度
    }
    return $(window).height() - $.fn.getGridTopHeight() - $.fn.getLayoutBottomHeight() - pageHeight - 6;
}
...
$.fn.jsonWhere = function (data, action) {
...
$.fn.InitTable = function (tb, actionUrl, dbQueryParams, tbColumns, showCheckBox) {
    tbColumns.unshift({
        title: '序号',
        align: 'center',
        formatter: function (value, row, index) {
            var pageSize = tb.bootstrapTable('getOptions').pageSize;     //通过table的#id 得到每页多少条
            var pageNumber = tb.bootstrapTable('getOptions').pageNumber; //通过table的#id 得到当前第几页
...
$.fn.tableRowValue = function (code) {
    var $table = $(this);
    var json = [];
