    ...
    DateRangePicker.prototype = {
        ...
        updateInputText: function () { 
            if (this.element.is('input') && !this.singleDatePicker) {
                var texts = this.element.parent().find('input');/*LGY修改 */
                if (texts.length == 2) {

                    texts[0].value = this.startDate.format(this.format);
                    texts[1].value = this.endDate.format(this.format);

                    $(texts[0]).trigger('change');
                    $(texts[1]).trigger('change');
                } else {  
